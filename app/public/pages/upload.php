...
<div class="up d-flex flex-column align-items-center justify-content-center"
    ...
    <?php 
    if (empty($_SESSION)) {
header('location:'. PAGES_ROOT . '/pages/index.php');
    exit();
} 
if (!empty($_FILES)){
$target_dir = "../pictures/";
...
// Check if file already exists
    if (file_exists($target_file)) {
        ...
        echo "<span id='alert' class='alert alert-warning mb-5'>File is too large.(>1MB)</span>";
        $uploadOk = 0;
    } elseif ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "webp") {
        echo "<span id='alert' class='alert alert-warning mb-5'>Sorry, only JPG, JPEG, PNG and WEBP files are allowed.</span>";
        $uploadOk = 0;
    } else {
